...
export default function ReconciliationPage() {
  ...
  const load = async () => {
    try {
      ...
      const url = queryString
        ? `/api/reconciliation?${queryString}`
        : "/api/reconciliation";
  ...
  const markReconciled = async (type: "invoice" | "payment", id: string) => {
    try {
      ...
      if (!res.ok)
        throw new Error(payload.error ?? "Failed to mark reconciled.");
      await load();
    } catch (err) {
      setError(
  ...
  const exportCsv = () => {
    const url = queryString
      ? `/api/reconciliation/export?${queryString}`
        ...
        <div className="glass-card-content flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">
              Match invoices with posted payments, track unmatched records, and
              close reconciliation rows.
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="glass-card p-0">
        <div className="border-b border-white/10 px-6 py-4">
          <h2 className="text-base font-semibold text-white">
            Invoices with Payment Matching
          </h2>
        </div>
        {loading ? (
              ...
              {invoices.length === 0 ? (
                ...
                invoices.map((row) => (
                  <TableRow
                    ...
                    className={
                      row.unmatched
                        ? "border-white/10 bg-rose-500/10 transition-colors hover:bg-rose-500/20"
                  ...
                  >
                    ...
                    <TableCell
                      className={`text-right font-semibold ${row.outstanding > 0 ? "text-rose-300" : "text-emerald-300"}`}
                    >
                    ...
                    <TableCell>
                      {row.matchedPayments.length === 0 ? (
                        ...
                        <div className="space-y-1 text-xs text-slate-400">
                          ...
                          {row.matchedPayments.length > 3 ? (
                            <p>+{row.matchedPayments.length - 3} more</p>
                          ) : null}
                        </div>
                      )}
                    </TableCell>
                    <TableCell>
                      <span
                        className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${row.reconciled ? "border border-emerald-500/30 bg-emerald-500/10 text-emerald-200" : "border border-white/20 bg-white/5 text-slate-400"}`}
                      >
          ...
          <TableBody>
            {unmatchedPayments.length === 0 ? (
              ...
              unmatchedPayments.map((row) => (
                <TableRow
                  key={row.id}
                  className="border-white/10 bg-amber-500/10 transition-colors hover:bg-amber-500/20"
                >
                  ...
                  <TableCell className="text-slate-300">
                    {row.salesOrderNumber ? `SO ${row.salesOrderNumber}` : "-"}
                  </TableCell>
                  <TableCell>
                    <span
                      className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${row.reconciled ? "border border-emerald-500/30 bg-emerald-500/10 text-emerald-200" : "border border-white/20 bg-white/5 text-slate-400"}`}
