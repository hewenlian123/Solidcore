...
type CategoryId =
  "all" | "flooring" | "doors" | "windows" | "mirrors" | "other";
...
function NewSalesOrderPageContent({ editOrderId }: { editOrderId?: string }) {
  ...
  const [docType, setDocType] = useState<"QUOTE" | "SALES_ORDER">(
    "SALES_ORDER",
  );
  ...
  const [discountInputByItemId, setDiscountInputByItemId] = useState<
    Record<string, string>
  >({});
  const [activeProductPickerItemId, setActiveProductPickerItemId] = useState<
    string | null
  >(null);
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const savingRef = useRef(false);
  const [partialStatusOrderId, setPartialStatusOrderId] = useState<
    string | null
  >(null);
  ...
  const [productListScroll, setProductListScroll] = useState({
    scrollTop: 0,
    clientHeight: 500,
  });
  ...
  const customerDropdownList = useMemo(() => {
    const list: (
      { type: "new" } | { type: "customer"; customer: SalesCustomer }
    )[] = [];
    list.push({ type: "new" });
    filteredCustomerOptions.forEach((c) =>
      list.push({ type: "customer", customer: c }),
    );
    return list;
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedCategory !== "all") {
      list = list.filter(
        (p) =>
          inferCategory(p.category ?? p.collection ?? p.brand ?? p.name) ===
          selectedCategory,
      );
    }
  ...
  const productListVisibleRange = useMemo(() => {
    ...
    const start = Math.max(
      0,
      Math.floor(scrollTop / PRODUCT_ROW_HEIGHT_PX) - overscan,
    );
    const visibleCount =
      Math.ceil(clientHeight / PRODUCT_ROW_HEIGHT_PX) + overscan * 2;
  ...
  const loadProducts = async (q = "") => {
    ...
    const res = await fetch(
      params.toString()
        ? `/api/sales-orders/products?${params.toString()}`
    ...
    if (editOrderId) {
      ...
      const load = async () => {
        try {
          ...
          const newItems: DraftItem[] = orderItems.map(
            (oi: {
              id: string;
              productId: string;
              variantId: string;
  ...
  useEffect(() => {
    Promise.all([loadCustomers(), loadProducts(), loadSalespeople()]).catch(
      (e) =>
    ...
    const loadDefaultTax = async () => {
      try {
        ...
        if (!taxRateOverridden)
          setTaxRate(String(Number.isFinite(rate) ? rate : 0));
      } catch {
        // keep defaults
      }
    };
  ...
  useEffect(() => {
    const timer = setTimeout(
      () =>
        void loadCustomers(customerQuery).catch((e) =>
          setError(
  ...
  const total =
    subtotal - Number(discount || 0) + Number(shipping || 0) + taxAmount;
  ...
  const applyBulkDiscount = () => {
    const raw = String(bulkDiscountPercent ?? "")
      .trim()
      .replace(/%/g, "");
  ...
  const handleQuickSkuAdd = () => {
    ...
    if (!query) return;
    const exact = quickSearchResults.find(
      (r) => String(r.sku ?? "").toLowerCase() === query,
    );
    const fuzzy =
      exact ??
  ...
  const validateSelectedLineQuantities = (selectedItems: DraftItem[]) => {
  ...
  const createOrder = async (confirmAfterCreate: boolean) => {
    ...
    try {
      ...
      const selectedItems = items.filter((item) => item.variantId);
      ...
      validateSelectedLineQuantities(selectedItems);
      const payload = {
        ...
        deliveryState:
          fulfillmentMethod === "DELIVERY"
            ? String(deliveryState || "").trim()
            : null,
        deliveryZip:
          fulfillmentMethod === "DELIVERY"
            ? String(deliveryZip || "").trim()
            : null,
        specialOrder,
        hidePrices,
        ...
        requestedDeliveryAt: requestedDeliveryAt
          ? new Date(requestedDeliveryAt).toISOString().slice(0, 10)
          : null,
      ...
      if (!orderId)
        throw new Error("Sales order response did not include an order ID.");
      if (confirmAfterCreate) {
        const nextStatus = docType === "QUOTE" ? "QUOTED" : "CONFIRMED";
        const statusRes = await fetch(`/api/sales-orders/${orderId}/status`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json", "x-user-role": role },
          body: JSON.stringify({
            status: nextStatus,
        ...
        if (!statusRes.ok || !statusPayload?.data?.id) {
          setPartialStatusOrderId(orderId);
          const detail =
            statusPayload?.error && typeof statusPayload.error === "string"
              ? ` ${statusPayload.error}`
              : "";
          throw new Error(
            docType === "QUOTE"
              ? `The Quote draft was created, but it could not be marked as quoted. Open the draft and try again.${detail}`
              : `The Sales Order draft was created, but confirmation failed. Open the draft and try again.${detail}`,
          );
        }
      }
      if (docType === "QUOTE") {
  ...
  const updateOrder = async (orderId: string) => {
    ...
    try {
      ...
      if (validItems.length === 0)
        throw new Error("Add at least one line item with a product variant.");
      validateSelectedLineQuantities(validItems);
      const headerPayload = {
        customerId: selectedCustomerId,
        docType,
        projectName,
      ...
      if (!patchRes.ok)
        throw new Error(patchPayload.error ?? "Failed to update sales order");
      ...
      for (const item of toUpdate) {
        ...
        const res = await fetch(
          `/api/sales-orders/${orderId}/items/${item.id}`,
          {
            method: "PATCH",
            headers: {
  ...
  const cleanProductDisplayName = (name: string) =>
    String(name ?? "")
      .replace(/\s*\(SKU[^)]*\)\s*/gi, "")
      .trim();
  const formatStockBadge = (stock: number) => {
    ...
    return {
      label: `${Number(stock).toFixed(0)} pcs`,
      className: "bg-emerald-500/20 text-emerald-300 border-emerald-400/30",
    };
  };
  const pageModeTitle = `${editOrderId ? "Edit" : "New"} ${docType === "QUOTE" ? "Quote" : "Sales Order"}`;
  const primaryActionLabel = editOrderId
    ? "Save Changes"
    : docType === "QUOTE"
      ? "Save Quote"
      : "Confirm Order";
  ...
  return (
    <div
      className="flex flex-1 min-h-0 flex-col overflow-hidden so-entry-page"
      style={{
    ...
    >
      {error && (
        <div
          role="alert"
          className="shrink-0 px-6 py-2 bg-rose-500/10 border-b border-rose-400/20 text-rose-200 text-sm"
        >
          <span>{error}</span>
          {partialStatusOrderId ? (
            <Link
              href={`/orders/${partialStatusOrderId}`}
      ...
      <div className="glass-card flex-1 min-h-0 overflow-hidden flex flex-col">
        <div className="glass-card-content flex flex-col flex-1 min-h-0 overflow-hidden p-3 sm:p-4 lg:p-6">
          {/* 1) Fixed top toolbar */}
          <header className="shrink-0 flex flex-wrap items-center gap-3 pb-4 border-b border-white/[0.08]">
            <div className="flex w-full min-w-0 items-center gap-3 lg:w-auto">
              <Link
                href="/orders"
                className="inline-flex h-10 shrink-0 items-center gap-1 rounded-lg px-2 text-sm font-medium text-slate-400 transition-colors hover:bg-white/[0.04] hover:text-white"
              >
              ...
              <span className="shrink-0 rounded-md border border-amber-400/20 bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-medium text-amber-400/90">
                Draft
              </span>
            </div>
            <div
              className="relative min-w-0 flex-[1_1_220px] lg:max-w-sm"
            ...
            >
              <input
                ...
                onBlur={() =>
                  setTimeout(() => setOpenCustomerDropdown(false), 150)
                }
                onKeyDown={(e) => {
                  if (
                    !openCustomerDropdown ||
                    customerDropdownList.length === 0
                  ) {
                  ...
                  if (e.key === "ArrowDown") {
                    e.preventDefault();
                    setCustomerDropdownHighlight(
                      (i) => (i + 1) % customerDropdownList.length,
                    );
                    return;
                  }
                  if (e.key === "ArrowUp") {
                    ...
                    setCustomerDropdownHighlight(
                      (i) =>
                        (i - 1 + customerDropdownList.length) %
                        customerDropdownList.length,
                    );
                    return;
                  }
                  if (e.key === "Enter") {
                    e.preventDefault();
                    const item =
                      customerDropdownList[customerDropdownHighlight];
                    if (!item) return;
                    if (item.type === "new") {
                    ...
                    if (item.type === "customer") {
                      ...
                      if (c.taxExempt) setTaxRate("0");
                      else
                        setTaxRate(String(Number(c.taxRate ?? defaultTaxRate)));
                      if (fulfillmentMethod === "DELIVERY")
                        setDeliveryAddress(
                          (prev) => prev || String(c.address ?? ""),
                        );
                      setOpenCustomerDropdown(false);
                    }
                    return;
                ...
                className="h-10 w-full px-3 rounded-lg border border-white/[0.08] bg-white/[0.04] text-white placeholder:text-slate-500 text-sm outline-none focus:ring-1 focus:ring-white/20"
              />
              {openCustomerDropdown && (
                    ...
                    if (item.type === "new") {
                      return (
                        <button
                          ...
                          onMouseEnter={() => setCustomerDropdownHighlight(idx)}
                          className={`w-full px-3 py-2 text-left text-xs font-medium ${idx === customerDropdownHighlight ? "bg-white/10 text-white" : "text-slate-400 hover:bg-white/5"}`}
                        >
                          + New Customer
                        </button>
                      );
                      ...
                      <button
                        ...
                        onMouseDown={(e) => {
                          ...
                          if (c.taxExempt) setTaxRate("0");
                            ...
                            setTaxRate(
                              String(Number(c.taxRate ?? defaultTaxRate)),
                            );
                          if (fulfillmentMethod === "DELIVERY")
                            setDeliveryAddress(
                              (prev) => prev || String(c.address ?? ""),
                        ...
                        onMouseEnter={() => setCustomerDropdownHighlight(idx)}
                        className={`w-full px-3 py-2 text-left text-xs truncate ${selectedCustomerId === c.id ? "bg-white/10 text-white" : idx === customerDropdownHighlight ? "bg-white/10 text-white" : "text-slate-200"}`}
                        title={c.phone ?? undefined}
                      >
                        <span className="block truncate">{c.name}</span>
                        {c.phone ? (
                          <span className="block truncate text-slate-500 text-[11px] mt-0.5">
                            {c.phone}
                          </span>
                        ) : null}
                      </button>
            ...
            <div
              className="flex h-10 shrink-0 rounded-lg border border-white/[0.08] bg-white/[0.04] p-0.5"
            ...
            >
              <button
                ...
                className={`min-w-[76px] px-3 rounded-md text-xs font-medium ${fulfillmentMethod === "PICKUP" ? "bg-white/10 text-white" : "text-slate-400 hover:text-white"}`}
              >
                Pickup
              </button>
              <button
                ...
                className={`min-w-[76px] px-3 rounded-md text-xs font-medium ${fulfillmentMethod === "DELIVERY" ? "bg-white/10 text-white" : "text-slate-400 hover:text-white"}`}
              >
                Delivery
              </button>
            </div>
            <button
              type="button"
              onClick={() => setOrderInfoModalOpen(true)}
              className="h-10 shrink-0 px-3 rounded-lg text-sm text-slate-300 hover:text-white border border-white/[0.08] hover:bg-white/[0.04]"
            >
              Details
            </button>
          </header>
          ...
          <div
            className="flex flex-1 min-h-0 flex-col gap-4 mt-4 overflow-y-auto lg:flex-row lg:overflow-hidden"
          ...
          >
            ...
            {productsPanelCollapsed ? (
              ...
              <aside className="order-2 w-full min-w-0 flex flex-col shrink-0 lg:order-1 lg:w-[35%] lg:min-h-0">
                {/* ProductsPanel: limited height; Search + Categories + ProductTableHeader (fixed) + ProductTableBody (scrolls, max 420px) */}
                <div className="rounded-xl border border-white/[0.08] bg-white/[0.04] overflow-hidden flex flex-col max-h-[360px] lg:max-h-none lg:h-full">
                  <div className="p-3 border-b border-white/[0.06] flex items-center gap-2 shrink-0">
                    <div className="relative flex-1 min-w-0">
                      ...
                      <input
                        ...
                        className="w-full h-10 pl-9 pr-3 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-slate-500 text-sm outline-none focus:ring-1 focus:ring-white/20"
                      />
                    </div>
                    <button
                      type="button"
                      onClick={() => setProductsPanelCollapsed(true)}
                      className="h-10 w-10 shrink-0 rounded-lg border border-white/[0.08] bg-white/[0.04] text-slate-400 hover:bg-white/[0.06] flex items-center justify-center"
                  ...
                  <div className="flex flex-col overflow-hidden min-h-0">
                    {filteredProducts.length === 0 ? (
                      ...
                      <>
                        <div className="sticky top-0 z-10 grid h-11 shrink-0 grid-cols-[minmax(0,1fr)_4.5rem_4rem_3rem] items-center border-b border-white/[0.06] bg-white/[0.04] text-[11px] font-semibold uppercase tracking-wider text-slate-400">
                          <span className="min-w-0 px-3">Product Name</span>
                          <span className="px-2 text-right">Price</span>
                          <span className="px-1 text-right">Stock</span>
                          <span className="pr-2 text-right">Add</span>
                        </div>
                        {/* Product list body — limited height, rows scroll inside; virtual list renders only visible rows */}
                        <div
                        ...
                        >
                          <div
                            style={{
                              height:
                                filteredProducts.length * PRODUCT_ROW_HEIGHT_PX,
                            }}
                          >
                            <div
                              className="w-full"
                              style={{
                                transform: `translateY(${productListVisibleRange.start * PRODUCT_ROW_HEIGHT_PX}px)`,
                              }}
                            >
                              {filteredProducts
                                .slice(
                                  productListVisibleRange.start,
                                  productListVisibleRange.end,
                                )
                                .map((product) => {
                                  const stock = Number(
                                  ...
                                  return (
                                    <div
                                      key={product.id}
                                      className="grid min-h-11 grid-cols-[minmax(0,1fr)_4.5rem_4rem_3rem] items-center border-b border-white/[0.04] text-sm hover:bg-white/[0.03]"
                                    >
                                      <div className="min-w-0 px-3 py-2">
                                        <span
                                          className="block truncate leading-tight text-slate-200"
                                          title={displayName}
                                        >
                                          {displayName}
                                        </span>
                                      </div>
                                      <div className="px-2 text-right font-medium tabular-nums text-slate-200">
                                        ${Number(product.price || 0).toFixed(2)}
                                      </div>
                                      <div className="px-1 text-right">
                                        <span className="inline-flex rounded border border-white/[0.08] bg-white/[0.06] px-1.5 py-0.5 text-xs font-medium leading-tight tabular-nums text-slate-400">
                                          {stock} {unit}
                                        </span>
                                      </div>
                                      <div className="flex justify-end pr-2">
                                        <button
                                          ...
                                          className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-white/[0.12] bg-white/[0.04] text-slate-400 transition-colors hover:border-emerald-500/50 hover:bg-emerald-600/80 hover:text-white"
                                          title="Add to cart"
            ...
            <main
              className={
                productsPanelCollapsed
                  ? "order-1 min-h-[520px] flex-1 min-w-0 flex flex-col overflow-hidden lg:order-2 lg:min-h-0"
                  : "order-1 min-h-[520px] w-full min-w-0 flex flex-col flex-1 overflow-hidden sm:min-h-[600px] lg:order-2 lg:w-[65%] lg:min-h-0"
              }
            >
              <div className="rounded-xl border border-white/[0.08] bg-white/[0.04] flex flex-col flex-1 min-h-0 overflow-hidden">
                <div className="p-3 border-b border-white/[0.06] flex flex-wrap items-center gap-3 shrink-0">
                  ...
                  >
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" &&
                        (e.preventDefault(), handleQuickSkuAdd())
                      }
                      placeholder="Scan SKU or product name"
                      aria-label="Product or SKU search"
                      className="w-full h-10 px-3 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white placeholder:text-slate-500 text-sm outline-none focus:ring-1 focus:ring-white/20"
                    />
                    {quickSearchOpen && (
                            ...
                            return (
                              ...
                              >
                                {product.imageUrl ? (
                                  ...
                                  <div className="h-8 w-8 rounded bg-white/[0.06] flex items-center justify-center text-slate-500 text-[10px] shrink-0">
                                    —
                                  </div>
                                )}
                                ...
                                <span className="text-white font-semibold text-xs tabular-nums shrink-0">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <input
                      ...
                      placeholder="%"
                      className="w-14 h-10 px-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm text-right outline-none focus:ring-1 focus:ring-white/20"
                    />
                    <button
                      type="button"
                      onClick={applyBulkDiscount}
                      className="h-10 px-3 rounded-lg border border-white/[0.08] bg-white/[0.04] text-slate-400 hover:bg-white/[0.08] text-xs font-medium"
                    >
                      Apply
                    </button>
                  </div>
                ...
                {/* CartBody: only this scrolls */}
                <div className="flex-1 min-h-0 overflow-y-auto">
                  {items.filter((i) => i.variantId).length === 0 ? (
                    <div className="flex min-h-full flex-col items-center justify-center py-12 px-4 text-center">
                      <ShoppingCart className="h-9 w-9 text-slate-500 mb-3" />
                      <p className="text-white font-medium text-sm mb-1">
                        Cart is empty
                      </p>
                      <p className="text-slate-500 text-xs">
                        Scan SKU or add products from the list
                      </p>
                    </div>
                  ) : (
                    <>
                      <table className="w-full table-fixed text-xs border-collapse">
                        <thead className="sticky top-0 bg-white/[0.04] border-b border-white/[0.06] z-10">
                          <tr className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                            <th className="text-left py-2 px-3 min-w-0">
                              Product
                            </th>
                            <th className="text-center py-2 px-1 w-32">Qty</th>
                            <th className="hidden text-right py-2 px-3 w-24 sm:table-cell">
                              Price
                            </th>
                            <th className="hidden text-right py-2 px-3 w-24 sm:table-cell">
                              Total
                                      ...
                                      {isActive && products.length > 0 && (
                                        <div className="absolute z-20 left-0 right-0 mt-1 max-h-36 overflow-y-auto rounded-md border border-white/[0.08] bg-slate-900/98 py-1 shadow-xl">
                                          <button
                                            ...
                                            className="w-full px-2 py-1.5 text-left text-[11px] text-slate-500 hover:bg-white/5"
                                          >
                                            Clear
                                          </button>
                                          {products.slice(0, 6).map((p) => (
                                            <button
                                              ...
                                              onMouseDown={(e) => {
                                                e.preventDefault();
                                                pickProductForItem(item.id, p);
                                              }}
                                              className="w-full flex items-center gap-2 px-2 py-1.5 text-left text-xs hover:bg-white/[0.06]"
                                            >
                              ...
                              >
                                <td className="py-2 px-3 min-w-0">
                                  ...
                                  <div className="mt-2 flex items-center gap-2 sm:hidden">
                                    ...
                                    <input
                                      ...
                                      aria-label={`Mobile unit price for ${item.productTitle || item.productSku}`}
                                      className="h-9 w-20 rounded-md border border-white/[0.08] bg-white/[0.04] px-2 text-right text-xs text-white outline-none focus:ring-1 focus:ring-white/20 tabular-nums"
                                    />
                                    <span className="ml-auto text-xs font-medium text-white tabular-nums">
                                      ${lineTotal.toFixed(2)}
                                    </span>
                                  </div>
                                </td>
                                <td className="py-2 px-1 text-center">
                                  <div className="flex items-center justify-center gap-1">
                                    <button
                                      type="button"
                                      aria-label={`Decrease quantity for ${item.productTitle || item.productSku}`}
                                      onClick={() =>
                                        updateItem(item.id, {
                                          quantity: String(
                                            Math.max(
                                      ...
                                      className="h-9 w-9 rounded-md border border-white/[0.1] text-slate-400 hover:bg-white/[0.06] text-sm"
                                    >
                                      −
                                    </button>
                                    <input
                                      ...
                                      aria-label={`Quantity for ${item.productTitle || item.productSku}`}
                                      className="w-12 h-9 rounded-md border border-white/[0.08] bg-white/[0.04] text-white text-center text-xs outline-none focus:ring-1 focus:ring-white/20"
                                    />
                                    <button
                                      type="button"
                                      aria-label={`Increase quantity for ${item.productTitle || item.productSku}`}
                                      onClick={() =>
                                        updateItem(item.id, {
                                          quantity: String(
                                            Number(item.quantity || 0) + 1,
                                ...
                                <td className="hidden py-2 px-3 text-right sm:table-cell">
                                  <input
                                    type="number"
                                    min="0"
                                    step="0.01"
                                    ...
                                    aria-label={`Unit price for ${item.productTitle || item.productSku}`}
                                    className="w-full max-w-[5rem] h-9 rounded-md border border-white/[0.08] bg-white/[0.04] text-white text-right text-xs px-2 outline-none focus:ring-1 focus:ring-white/20 tabular-nums"
                                  />
                                </td>
                                <td className="hidden py-2 px-3 text-right font-medium text-white tabular-nums sm:table-cell">
                                  ${lineTotal.toFixed(2)}
                                </td>
                                <td className="py-2 px-1 text-center">
                                  <button
                                    type="button"
                                    onClick={() => removeItem(item.id)}
                                    aria-label={`Remove ${item.productTitle || item.productSku}`}
                                    className="h-9 w-9 rounded-md text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 inline-flex items-center justify-center transition-colors"
                                  >
                      ...
                      <div className="p-3 border-t border-white/[0.04]">
                        <button
                          type="button"
                          onClick={() => addLine()}
                          className="w-full h-10 text-xs text-slate-500 hover:text-white hover:bg-white/[0.04] rounded-lg font-medium"
                        >
                          + Add line
                        </button>
                      </div>
              ...
              <div
                className="shrink-0 mt-3 pt-4 border-t border-white/[0.08] bg-[#0F172A]/95 rounded-b-xl flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4"
                data-testid="primary-action-area"
              >
                <div className="space-y-1 text-sm" data-testid="totals-summary">
                ...
                >
                  <button
                    ...
                    className="btn-primary-emerald col-span-2 h-[44px] px-5 rounded-lg text-white text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-50 sm:col-span-1"
                  ...
                  {!editOrderId ? (
                    <button
                      ...
                      className="h-11 px-4 rounded-lg border border-white/[0.12] text-slate-300 hover:bg-white/[0.06] text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-50"
                    >
                  ...
                  <button
                    ...
                    className="h-11 px-4 rounded-lg border border-white/[0.08] text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 text-sm font-medium flex items-center justify-center gap-2"
                  >
      ...
      <Fragment>
        ...
        {/* Lower-frequency order metadata */}
        {orderInfoModalOpen && (
          <div
            className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md p-4"
            onClick={() => setOrderInfoModalOpen(false)}
          >
            <div
              className="so-modal-shell w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="p-5 border-b border-white/[0.08] flex items-center justify-between shrink-0">
                <h3 className="text-lg font-bold text-white tracking-tight">
                  {docType === "QUOTE" ? "Quote details" : "Order details"}
                </h3>
                <button
                  type="button"
                  onClick={() => setOrderInfoModalOpen(false)}
              ...
              <div className="overflow-y-auto p-5 space-y-4 flex-1">
                <div>
                  <label className="so-entry-text-secondary text-xs uppercase tracking-[0.12em] font-semibold block mb-2">
                    Salesperson
                  </label>
                  <select
                    value={salespersonName}
                    onChange={(e) => setSalespersonName(e.target.value)}
                    className="w-full h-9 px-3 rounded-lg bg-white/[0.05] border border-white/[0.10] text-white text-sm outline-none backdrop-blur-xl focus:ring-2 focus:ring-cyan-400/30"
                  >
                ...
                {fulfillmentMethod === "DELIVERY" && (
                  <div className="space-y-2">
                    <label className="so-entry-text-secondary text-xs uppercase tracking-[0.12em] font-semibold block">
                      Delivery Address
                    </label>
                    <input
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <input
                        ...
                        placeholder="City"
                        className="h-9 px-2 rounded-lg bg-white/[0.05] border border-white/[0.10] text-white placeholder:text-white/40 text-xs outline-none backdrop-blur-xl focus:ring-2 focus:ring-cyan-400/30"
                      />
                      <input
                        value={deliveryState}
                ...
                <div className="rounded-lg border border-white/[0.08] bg-white/[0.03] p-3 space-y-3">
                  <p className="so-entry-text-secondary text-[11px] font-semibold uppercase tracking-wider">
                    Schedule
                  </p>
                  <div>
                    <label className="so-entry-text-secondary text-[11px] uppercase tracking-wider font-medium block mb-1">
                      Order Date
                    </label>
                    <input
                      type="date"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-3 rounded-xl bg-white/[0.05] border border-white/[0.10] text-white placeholder:text-white/40 text-sm outline-none backdrop-blur-xl focus:ring-2 focus:ring-cyan-400/30 resize-none"
                  />
                </div>
              </div>
              <div className="p-5 border-t border-white/[0.08] shrink-0">
                <button
                  type="button"
                  onClick={() => setOrderInfoModalOpen(false)}
                  className="w-full h-11 rounded-xl bg-white/[0.08] border border-white/20 text-white text-sm font-medium hover:bg-white/[0.14] transition-all duration-200"
                >
                  Done
                </button>
              </div>
        ...
        {historyModalOpen && (
          ...
          >
            <div
              className="so-modal-shell w-full max-w-2xl max-h-[80vh] flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
                ...
                {loadingHistory ? (
                  ...
                  <ul className="space-y-2">
                    {recentOrders.map((order) => (
                      <li
                        key={order.id}
                        className="flex items-center justify-between gap-4 py-3 px-4 rounded-xl bg-white/[0.05] border border-white/[0.08]"
                      >
                        <div>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              if (!res.ok)
                                throw new Error(
                                  body.error ?? "Failed to load order",
                                );
                              const data = body.data;
                              if (data?.customer?.id) {
                                setSelectedCustomerId(data.customer.id);
        ...
        {printModalOpen && (
          ...
          >
            <div
              className="w-full max-w-2xl max-h-[90vh] rounded-2xl border border-white/[0.1] bg-slate-900/95 backdrop-blur-xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="p-5 border-b border-white/[0.08] flex items-center justify-between">
                <h3 className="text-lg font-bold text-white tracking-tight">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    onClick={() => {
                      ...
                      const styles =
                        "table { width:100%; border-collapse:collapse; } th, td { padding:6px 8px; text-align:left; border-bottom:1px solid #ccc; } th { font-weight:600; } .text-right { text-align:right; }";
                      const html =
                        `<!DOCTYPE html><html><head><title>Sales Order</title><style>${styles}</style></head><body style="font-family:sans-serif;padding:24px;color:#111;">${el.innerHTML
                          .replace(
                            /text-white|text-slate-400|bg-white\/\[0\.03\]|border-white\/\[0\.08\]|rounded-xl|rounded-lg/g,
                            "",
                          )
                          .replace(/text-slate-200|text-slate-400/g, "")
                          .replace(/className="[^"]*"/g, "")
                          .replace(/class="[^"]*"/g, "")}` + "</body></html>";
              ...
              <div
                ...
                className="overflow-y-auto p-6 text-sm text-white bg-white/[0.03] m-4 rounded-xl border border-white/[0.08]"
              >
                <p className="font-bold text-lg mb-2">Sales Order — Draft</p>
                <p>
                  <strong>Customer:</strong>{" "}
                  {(selectedCustomer?.name ?? customerQuery) || "—"}
                </p>
                    ...
                    {items
                      ...
                      .map((item) => {
                        ...
                        return (
                          ...
                          >
                            <td className="py-2">
                              {item.productTitle || item.productSku}
                            </td>
