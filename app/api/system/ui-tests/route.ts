...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (sessionRes.ok && (sessionData?.user ?? sessionData?.authenticated)) {
      ...
      results.push({
        name: "login",
        status: "passed",
        message: "Session endpoint OK (login flow available)",
      });
    }
  } catch (e) {
    results.push({
  ...
  try {
    ...
    if (orderRes.status === 400 || orderRes.status === 401) {
      results.push({
        name: "createOrder",
        status: "passed",
        message: "API reachable, validation/auth as expected",
      });
    } else if (orderRes.ok) {
      ...
      results.push({
        name: "createOrder",
        status: "passed",
        message: `API returned ${orderRes.status}`,
      });
    }
  } catch (e) {
    results.push({
  ...
  try {
    ...
    if (productsRes.ok) {
      ...
      results.push({
        name: "addProduct",
        status: "failed",
        message: `Products API ${productsRes.status}`,
      });
    }
  } catch (e) {
    results.push({
  ...
  try {
    const invoicesRes = await fetch(`${BASE}/api/invoices?limit=1`, {
      headers: { cookie: cookies },
    });
    if (invoicesRes.ok || invoicesRes.status === 401) {
      ...
      results.push({
        name: "saveInvoice",
        status: "failed",
        message: `Invoices API ${invoicesRes.status}`,
      });
    }
  } catch (e) {
    results.push({
  ...
  try {
    ...
    const fileOk =
      ...
      health?.services?.fileUpload === "connected";
    if (healthRes.ok && (fileOk || health?.status === "ok")) {
      ...
      results.push({
        name: "uploadFile",
        status: "passed",
        message: "Health check reachable",
      });
    }
  } catch (e) {
    results.push({
