...
export default function AfterSalesReturnDetailPage() {
    ...
    try {
      ...
      setItemDrafts(
        (payload.data?.items ?? []).map((item: any) => ({
          id: String(item.id),
          qtyPurchased:
            item.qtyPurchased != null ? String(item.qtyPurchased) : null,
  ...
  const nextStatus = useMemo(
    () => (data ? (STATUS_FLOW[data.status][0] ?? null) : null),
    [data],
  );
        ...
        <div className="glass-card-content flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Customer: {data.customer.name} · SO:{" "}
              {data.salesOrder?.orderNumber ?? "-"} · Invoice:{" "}
              {data.invoice?.invoiceNumber ?? "-"}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded border border-white/10 bg-white/5 px-2 py-1 text-xs font-semibold text-slate-300 backdrop-blur-xl">
              {data.status}
            </span>
            <button
              type="button"
            ...
            <button
              type="button"
              onClick={createPdfViaApiReturns}
              className="ios-secondary-btn h-9 px-3 text-xs"
              disabled={saving}
            >
              Print / PDF
            ...
            {nextStatus ? (
              <button
                type="button"
                onClick={moveStatusViaApiReturns}
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-white">
                ${Number(data.refundTotal).toFixed(2)}
              </p>
            </div>
          </div>
                  ...
                  data.items.map((item) => (
                    ...
                    >
                      ...
                      <td className="py-2 pr-4 text-right">
                        {Number(item.qtyPurchased ?? 0).toFixed(2)}
                      </td>
                      <td className="py-2 pr-4 text-right">
                        {Number(item.qtyReturn).toFixed(2)}
                      </td>
                      <td className="py-2 pr-4 text-right">
                        ${Number(item.unitPrice).toFixed(2)}
                      </td>
                      <td className="py-2 pr-4 text-right">
                        ${Number(item.lineRefund).toFixed(2)}
                      </td>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setItemDrafts((prev) =>
                          prev.map((row) =>
                            row.id === item.id
                              ? { ...row, condition: event.target.value }
                              : row,
                          ),
                        )
                      }
