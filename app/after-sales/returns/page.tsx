...
function AfterSalesReturnsContent() {
  ...
  const loadCreateData = async (nextForm: typeof form) => {
    ...
    if (nextForm.salesOrderId)
      params.set("salesOrderId", nextForm.salesOrderId);
    if (nextForm.invoiceId) params.set("invoiceId", nextForm.invoiceId);
    const res = await fetch(
      `/api/after-sales/returns/create-data?${params.toString()}`,
      {
        cache: "no-store",
        headers: { "x-user-role": role },
      },
  ...
  useEffect(() => {
    ...
    void loadCreateData(nextForm).catch((err) =>
      setError(
        err instanceof Error ? err.message : "Failed to load create options",
      ),
    );
  }, [searchParams, role]);
  ...
  const salesOrderOptions = useMemo(
    () =>
      (createData?.salesOrders ?? []).filter(
        (row) => !form.customerId || row.customerId === form.customerId,
      ),
  ...
  const invoiceOptions = useMemo(() => {
    ...
    return list.filter((row) => {
      if (form.salesOrderId && row.salesOrderId !== form.salesOrderId)
        return false;
      if (
        form.customerId &&
        row.customerId &&
        row.customerId !== form.customerId
      )
        return false;
      return true;
    });
  ...
  const submitCreate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const draftByLineItemId = new Map(
        draftItems.map((item) => [item.lineItemId, item]),
      );
        ...
        <div className="glass-card-content flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Review approvals, received items, refund status, and return
              history.
            </p>
          </div>
          <button
            ...
            className="ios-primary-btn h-10 px-3 text-sm"
          >
            Create Return
          </button>
        </div>
                ...
                rows.map((row) => (
                  ...
                  >
                    ...
                    <TableCell className="text-right">
                      ${Number(row.refundTotal).toFixed(2)}
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <Link
                        href={`/after-sales/returns/${row.id}`}
                        ...
                        (createData?.items ?? []).map((item, idx) => (
                          ...
                          >
                            ...
                            <td className="py-1 pr-2 text-right">
                              {Number(item.qtyPurchased).toFixed(2)}
                            </td>
...
export default function AfterSalesReturnsPage() {
  return (
    <Suspense
      fallback={
        <section className="glass-card p-8 text-sm text-slate-400">
          Loading returns...
        </section>
      }
    >
