...
export default function InvoicePaymentDetailPage() {
  ...
  const deletePayment = async () => {
    ...
    const ok = window.confirm(
      "Delete this payment? This will void it and update invoice balance.",
    );
    if (!ok) return;
    try {
      setSaving(true);
        ...
        <div className="glass-card-content flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">
              Invoice: {data.invoice.invoiceNumber} · SO:{" "}
              {data.invoice.salesOrder?.orderNumber ?? "-"}
            </p>
          </div>
        ...
        <article className="glass-card space-y-2 p-4 text-sm">
          <div className="glass-card-content">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white/90">{data.payment.method}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Type</span>
              <span className="text-white/90">{data.payment.paymentType}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white/90">
                {formatDateTime(data.payment.receivedAt)}
              </span>
            </div>
            <div className="flex justify-between">
        ...
        <article className="glass-card space-y-2 p-4 text-sm">
          <div className="glass-card-content">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white/90">{data.invoice.status}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white/90">
                ${Number(data.paidTotal).toFixed(2)}
              </span>
            </div>
