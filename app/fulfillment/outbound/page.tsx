...
const statusBadge = (status: string) => {
  const key = status.toUpperCase();
  if (key === "COMPLETED" || key === "DELIVERED" || key === "PICKED_UP")
...
export default function FulfillmentOutboundPage() {
  ...
  const load = async () => {
    try {
      ...
      if (!res.ok)
        throw new Error(payload.error ?? "Failed to load outbound queue");
      setRows(payload.data ?? []);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to load outbound queue",
      );
    }
  };
  ...
  const setStatus = async (
    row: OutboundRow,
    status: "ready" | "out_for_delivery" | "picked_up" | "delivered",
  ) => {
    try {
      setBusyId(row.id);
      setError(null);
      const endpoint =
        status === "ready"
          ? `/api/fulfillments/${row.id}`
          : `/api/fulfillments/${row.id}/status`;
      ...
      <header className="glass-card p-8">
        <div className="glass-card-content">
          ...
          <p className="mt-2 text-sm text-slate-400">
            All non-completed and non-cancelled fulfillment tasks.
          </p>
        </div>
      </header>
                    ...
                    <TableCell>
                      {typeof row.itemCount === "number" ? (
                        <div className="space-y-0.5 text-xs">
                          ...
                          {typeof row.itemsCompleted === "number" &&
                          row.itemCount > 0 ? (
                            <div className="text-slate-400">
                              {row.itemsCompleted}/{row.itemCount} complete ·{" "}
                              {Math.round(
                                (row.itemsCompleted /
                                  Math.max(row.itemCount, 1)) *
                                  100,
                              )}
                              %
                            </div>
                          ...
                          <>
                            {row.status.toUpperCase() === "READY" ? (
                              <button
                                ...
                                disabled={busyId === row.id}
                                onClick={() =>
                                  setStatus(row, "out_for_delivery")
                                }
