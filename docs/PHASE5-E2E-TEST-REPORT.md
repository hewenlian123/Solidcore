...
| 6 | Mark Ready | **PASS** | Warehouse Operations (`/warehouse`) groups Pickup and Delivery into Needs Ready, Ready, and Delivery In Delivery sections. "Mark Ready" uses `PATCH /api/fulfillments/[id]` with status READY and does not update fulfilled quantities or inventory. |
| 7 | Generate Preparation List | **PASS** | The existing preparation PDF remains available through the fulfillment PDF endpoint. User-facing labels now say "Preparation List"; no standalone pre-handoff module is exposed. |
...
- **Fulfillment:** DRAFT/SCHEDULED → READY → OUT_FOR_DELIVERY (delivery) or PICKED_UP/COMPLETED. Final statuses: DELIVERED, PICKED_UP, COMPLETED trigger inventory deduction.
...
1. **SO line fulfillQty vs fulfillment item fulfilledQty:** Fulfillment item `fulfilledQty` drives inventory deduction when final fulfillment occurs. Sales order line `fulfillQty` is updated only when SO line is explicitly PATCHed (`/api/sales-orders/[id]/items/[itemId]`) or through approved fulfillment completion helpers. **Effect:** SO status may remain READY or CONFIRMED even after fulfillment is COMPLETED and inventory deducted if a future flow bypasses the canonical helpers. Inventory and fulfillment state are correct; SO status may lag until SO items are updated elsewhere.
