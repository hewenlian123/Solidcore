...
function assertSafeDatabase() {
  ...
  if (!rawUrl)
    throw new Error("DATABASE_URL is required for payment allocation tests.");
  const url = new URL(rawUrl);
  const isLocal = ["localhost", "127.0.0.1", "::1"].includes(url.hostname);
  const isExplicitTestDb = /test/i.test(url.pathname);
  if (!isLocal && !isExplicitTestDb) {
    throw new Error(
      "Refusing to run payment allocation tests against a non-local, non-test database.",
    );
  }
}
...
async function createFixture(
  label: string,
  overrides: Partial<{ total: number; status: string }> = {},
): Promise<Fixture> {
...
async function postOrderPayment(
  ...
  const payment = Array.isArray(body.data?.payments)
...
test.describe.serial("canonical payment allocation authority", () => {
  ...
  test("order-level payment remains unallocated and does not reduce invoice paid or balance", async ({
    ...
    expectMoney(
      detail.balanceDue,
      95,
      "invoice balance ignores unallocated order payment",
    );
    expectMoney(
      detail.unallocatedPaymentTotal,
      25,
      "invoice exposes unallocated order payment separately",
    );
    expect(detail.payments).toHaveLength(0);
    expect(detail.unallocatedPayments).toHaveLength(1);

    const invoiceList = await request.get(
      `/api/invoices?salesOrderId=${fixture.salesOrderId}`,
      {
        headers: authHeaders(),
      },
    );
    ...
    expectMoney(
      invoiceListBody.data[0].paidTotal,
      0,
      "invoice list paid total",
    );
    expectMoney(invoiceListBody.data[0].balanceDue, 95, "invoice list balance");

    const customerInvoices = await request.get(
      `/api/customers/${fixture.customerId}/invoices`,
      {
        headers: authHeaders(),
      },
    );
    ...
    expectMoney(
      customerBody.data[0].paidTotal,
      0,
      "customer invoice paid total",
    );
    ...
    const pdfResponse = await request.get(
      `/api/pdf/invoice/${fixture.invoiceId}`,
      { headers: authHeaders() },
    );
    expect(pdfResponse.status()).toBe(200);
    const pdfText = extractPdfContentText(
      Buffer.from(await pdfResponse.body()),
    );
    expect(pdfText).toContain("95.00");
  });

  test("explicit allocation preserves the payment id and creates no second receipt", async ({
    request,
  }) => {
    ...
    expectMoney(
      before.payments[0].amount,
      40,
      "unallocated payment amount before allocation",
    );
    expectMoney(
      before.order.paidAmount,
      40,
      "sales order paid before allocation",
    );
    expectMoney(
      before.order.balanceDue,
      60,
      "sales order balance before allocation",
    );
    ...
    expectMoney(
      detail.unallocatedPaymentTotal,
      40,
      "unallocated total before allocation",
    );
    expect(detail.payments).toHaveLength(0);
    expect(detail.unallocatedPayments).toHaveLength(1);

    const receiptBefore = await request.get(`/api/pdf/payment/${paymentId}`, {
      headers: authHeaders(),
    });
    expect(receiptBefore.status()).toBe(200);

    const allocated = await allocate(request, fixture, paymentId);
    expect(allocated.response.status(), JSON.stringify(allocated.body)).toBe(
      200,
    );
    ...
    expect(paymentAfter).toEqual({
      ...paymentBefore,
      invoiceId: fixture.invoiceId,
    });
    expectMoney(
      after.order.paidAmount,
      before.order.paidAmount,
      "sales order paid unchanged by allocation",
    );
    expectMoney(
      after.order.balanceDue,
      before.order.balanceDue,
      "sales order balance unchanged by allocation",
    );
    expect(after.invoice.subtotal).toBe(before.invoice.subtotal);
    expect(after.invoice.discountAmount).toBe(before.invoice.discountAmount);
    expect(after.invoice.taxAmount).toBe(before.invoice.taxAmount);
    ...
    expectMoney(
      detail.unallocatedPaymentTotal,
      0,
      "unallocated total after allocation",
    );
    expect(detail.payments).toHaveLength(1);
    expect(detail.unallocatedPayments).toHaveLength(0);

    const receiptAfter = await request.get(`/api/pdf/payment/${paymentId}`, {
      headers: authHeaders(),
    });
  ...
  test("repeated and concurrent same-payment allocation is idempotent", async ({
    request,
  }) => {
    ...
    expectMoney(
      before.order.paidAmount,
      30,
      "sales order paid before concurrent allocation",
    );
    expectMoney(
      before.order.balanceDue,
      50,
      "sales order balance before concurrent allocation",
    );
    ...
    expectMoney(
      state.order.paidAmount,
      before.order.paidAmount,
      "order paid unchanged after repeated allocation",
    );
    expectMoney(
      state.order.balanceDue,
      before.order.balanceDue,
      "order balance unchanged after repeated allocation",
    );
    const detail = await invoiceApi(request, fixture.invoiceId);
    ...
    expectMoney(
      detail.balanceDue,
      50,
      "invoice balance after repeated allocation",
    );
  });
  ...
  test("wrong order, void invoice, over-balance, and already-allocated payments are rejected without mutation", async ({
    ...
    const wrongPayment = await createPayment({
      salesOrderId: wrongSource.salesOrderId,
      amount: 10,
    });
    ...
    expect(
      wrongResult.response.status(),
      JSON.stringify(wrongResult.body),
    ).toBe(400);
    expect(wrongResult.body.error).toContain("different sales order");
    expect(
      (
        await prisma.salesOrderPayment.findUniqueOrThrow({
    ...
    const voided = await createFixture("VOIDED", {
      total: 100,
      status: "void",
    });
    const voidPayment = await createPayment({
    ...
    expect(voidResult.response.status(), JSON.stringify(voidResult.body)).toBe(
      400,
    );
    ...
    const overPayment = await createPayment({
      salesOrderId: over.salesOrderId,
      amount: 60,
    });
    const overBefore = await captureFinancialState(over);
    const overResult = await allocate(request, over, overPayment.id);
    expect(overResult.response.status(), JSON.stringify(overResult.body)).toBe(
      400,
    );
    ...
    const currentTarget = await createFixture("ALLOCATED-TARGET", {
      total: 100,
    });
    ...
    const allocatedResult = await allocate(
      request,
      currentTarget,
      allocatedPayment.id,
    );
    expect(
      allocatedResult.response.status(),
      JSON.stringify(allocatedResult.body),
    ).toBe(409);
    expect(allocatedResult.body.error).toContain("already allocated");
    expect(
      (
        await prisma.salesOrderPayment.findUniqueOrThrow({
          where: { id: allocatedPayment.id },
        })
      ).invoiceId,
    ).toBe(otherInvoice.invoiceId);
  ...
  test("removed store credit endpoint cannot mutate invoice allocation state", async ({
    ...
    const creditResponse = await request.post(
      `/api/invoices/${fixture.invoiceId}/apply-store-credit`,
      {
        headers: authHeaders(),
        data: { amount: 20 },
      },
    ...
    expectMoney(
      detail.paidTotal,
      30,
      "invoice paid remains direct payment only",
    );
    expectMoney(detail.balanceDue, 70, "invoice balance remains unchanged");
    expectMoney(
      detail.unallocatedPaymentTotal,
      10,
      "unallocated order payment remains separate",
    );
    expect(
      detail.payments.filter(
        (payment: { status: string }) => payment.status === "POSTED",
      ),
    ).toHaveLength(1);
  });
  ...
  test("invoice page visibly separates unallocated order payment and applies it from the existing receipt", async ({
    ...
    const cookieValue = createSessionCookie().replace(
      /^solidcore_session=/,
      "",
    );
    await page
      .context()
      .addCookies([
        { name: "solidcore_session", value: cookieValue, url: baseUrl() },
      ]);
    ...
    await expect(page.locator("main .ios-primary-btn")).toHaveCount(1);
    await expect(
      page.getByRole("heading", { name: "Allocated Invoice Payments" }),
    ).toBeVisible({
      timeout: 15_000,
    });
    await expect(
      page.getByText("No payments are allocated to this invoice yet."),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Unallocated Order Payments" }),
    ).toBeVisible();
    await expect(page.getByText("Available: $25.00")).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Apply to Invoice" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("link", { name: "Open Payment" }),
    ).toBeVisible();

    const persisted = await prisma.salesOrderPayment.findUniqueOrThrow({
      where: { id: payment.id },
    });
