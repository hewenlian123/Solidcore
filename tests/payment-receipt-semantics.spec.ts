...
function assertSafeDatabase() {
  ...
  if (!rawUrl) throw new Error("DATABASE_URL is required for receipt semantics tests.");
  const url = new URL(rawUrl);
  const isLocal = ["localhost", "127.0.0.1", "::1"].includes(url.hostname);
  const isExplicitTestDb = /test/i.test(url.pathname);
  if (!isLocal && !isExplicitTestDb) {
    throw new Error("Refusing to run receipt semantics tests against a non-local, non-test database.");
  }
}
...
async function createOrderAndInvoice() {
  ...
  return { customerId: customer.id, invoiceId: invoice.id, invoiceNumber, salesOrderId: order.id, salesOrderNumber: orderNumber };
}
...
async function postOrderPayment(
  ...
  const payment = await prisma.salesOrderPayment.findFirstOrThrow({
    where: { referenceNumber: args.reference, salesOrderId: fixture.salesOrderId },
...
async function createFixture(request: APIRequestContext): Promise<Fixture> {
  ...
  const unallocatedDepositPaymentId = await postOrderPayment(request, partialFixture, {
    amount: 40,
    reference: `PH4A42-UNALLOC-${runId}`,
    type: "DEPOSIT",
  });
  const allocatedDepositPaymentId = await postOrderPayment(request, partialFixture, {
    amount: 50,
  ...
  const voidedDepositPaymentId = await postOrderPayment(request, partialFixture, {
    amount: 25,
...
async function orderApiPayment(request: APIRequestContext, fixture: Fixture, paymentId: string) {
  const response = await request.get(`/api/sales-orders/${fixture.salesOrderId}`, { headers: authHeaders(null) });
  const body = await response.json().catch(() => ({}));
  expect(response.status(), JSON.stringify(body)).toBe(200);
  const payment = body.data?.payments?.find((row: { id: string }) => row.id === paymentId);
...
async function receiptPageText(page: Page, fixture: Fixture, paymentId: string) {
  ...
  await page.goto(`/sales-orders/${fixture.salesOrderId}/payments/${paymentId}/receipt`);
...
async function paymentPdfText(request: APIRequestContext, paymentId: string, asDownload = false) {
  const response = await request.get(`/api/pdf/payment/${paymentId}${asDownload ? "?download=true" : ""}`, {
    headers: authHeaders(null),
  });
  expect(response.status()).toBe(200);
  expect(response.headers()["content-type"]).toContain("application/pdf");
  expect(response.headers()["content-disposition"]).toContain(`payment-${paymentId.slice(0, 8)}.pdf`);
  if (asDownload) expect(response.headers()["content-disposition"]).toContain("attachment");
...
test.describe.serial("payment receipt and PDF semantics", () => {
  ...
  test("unallocated DEPOSIT receipt shows Deposit and no invoice number", async ({ page, request }) => {
    const payment = await orderApiPayment(request, fixture, fixture.unallocatedDepositPaymentId);
    expect(payment).toMatchObject({ invoiceId: null, paymentType: "DEPOSIT", status: "POSTED" });

    const text = await receiptPageText(page, fixture, fixture.unallocatedDepositPaymentId);
    ...
    const pdfText = await paymentPdfText(request, fixture.unallocatedDepositPaymentId);
  ...
  test("allocated DEPOSIT receipt preserves identity and shows invoice allocation", async ({ page, request }) => {
    const before = await capturePaymentIdentity(fixture.allocatedDepositPaymentId);
    ...
    const payment = await orderApiPayment(request, fixture, fixture.allocatedDepositPaymentId);
    expect(payment).toMatchObject({ invoiceId: fixture.invoiceId, paymentType: "DEPOSIT", status: "POSTED" });

    const text = await receiptPageText(page, fixture, fixture.allocatedDepositPaymentId);
    expect(text).toContain("Payment Type: Deposit");
    expect(text).toContain(`Allocation: Applied to Invoice ${fixture.invoiceNumber}`);
    expect(text).toContain(`Sales Order #: ${fixture.salesOrderNumber}`);
    expect(text).toContain(fixture.allocatedDepositPaymentId.slice(0, 8));

    const pdfText = await paymentPdfText(request, fixture.allocatedDepositPaymentId);
    expect(pdfText).toContain("Payment Type: Deposit");
    expect(pdfText).toContain(`Allocation: Applied to Invoice ${fixture.invoiceNumber}`);
    expect(pdfText).toContain(fixture.salesOrderNumber);
    expect(pdfText).toContain("50.00");

    expect(await capturePaymentIdentity(fixture.allocatedDepositPaymentId)).toEqual(before);
  });

  test("FINAL Payment and VOIDED Deposit receipts show correct semantics", async ({ page, request }) => {
    const finalText = await receiptPageText(page, fixture, fixture.finalPaymentId);
    ...
    const voidedBefore = await capturePaymentIdentity(fixture.voidedDepositPaymentId);
    expect(voidedBefore).toMatchObject({
      invoiceId: fixture.invoiceId,
      paymentType: "DEPOSIT",
      status: "VOIDED",
    });
    const voidedText = await receiptPageText(page, fixture, fixture.voidedDepositPaymentId);
    expect(voidedText).toContain("Payment Type: Deposit");
    expect(voidedText).toContain(`Allocation: Applied to Invoice ${fixture.invoiceNumber}`);
    expect(voidedText).toContain("Status: Voided");

    const voidedPdf = await paymentPdfText(request, fixture.voidedDepositPaymentId);
    expect(voidedPdf).toContain("Payment Type: Deposit");
    expect(voidedPdf).toContain(`Allocation: Applied to Invoice ${fixture.invoiceNumber}`);
    expect(voidedPdf).toContain("Status");
    expect(voidedPdf).toContain("Voided");
    expect(await capturePaymentIdentity(fixture.voidedDepositPaymentId)).toEqual(voidedBefore);
  });

  test("page, order API, and PDF values agree for one payment", async ({ page, request }) => {
    const apiPayment = await orderApiPayment(request, fixture, fixture.allocatedDepositPaymentId);
    const pageText = await receiptPageText(page, fixture, fixture.allocatedDepositPaymentId);
    const pdfText = await paymentPdfText(request, fixture.allocatedDepositPaymentId);
  ...
  test("receipt PDF endpoint returns inline and download PDF responses", async ({ request }) => {
  ...
  test("receipt page load, refresh, and PDF reads perform no writes", async ({ page, request }) => {
