...
function SalesTrendTooltip({
  ...
  return (
    <div
      className="min-w-[156px] rounded-xl border border-white/55 bg-[rgba(255,255,255,0.72)] p-3 shadow-[0_12px_30px_rgba(15,23,42,0.10),0_2px_8px_rgba(15,23,42,0.04)] backdrop-blur-[14px]"
    ...
    >
      ...
      <div className="mt-2 space-y-1.5">
        <div className="flex items-center justify-between gap-5 text-[12px] text-slate-700">
          ...
          <span className="font-semibold">${revenue.toLocaleString("en-US")}</span>
        </div>
        <div className="flex items-center justify-between gap-5 text-[12px] text-slate-700">
          <span className="inline-flex items-center gap-1.5">
          ...
          <span className="font-semibold">{orders.toLocaleString("en-US")}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <article className="glass-card glass-card-strong px-6 py-5">
      <div className="glass-card-content mb-5 flex items-center justify-between">
        <div>
          <h3 className="text-[19px] font-semibold text-slate-900">Sales Trend</h3>
          <p className="text-xs text-slate-500">Revenue & Orders</p>
        </div>
        <div className="inline-flex rounded-full border border-slate-200/70 bg-white p-1">
          <button
            type="button"
            onClick={() => setMode("revenue")}
            className={`rounded-full px-4 py-1.5 text-xs font-medium transition duration-200 hover:scale-[1.01] ${mode === "revenue" ? "bg-slate-900 text-white" : "text-slate-600"}`}
          >
            Revenue
          </button>
          <button
            ...
            className={`rounded-full px-4 py-1.5 text-xs font-medium transition duration-200 hover:scale-[1.01] ${mode === "orders" ? "bg-slate-900 text-white" : "text-slate-600"}`}
          >
            Orders
          </button>
        </div>
      ...
      <div ref={chartContainerRef} className="glass-card-content relative h-[300px]">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart key={`trend-${mode}`} data={chartData} margin={{ left: 2, right: 2, top: 10, bottom: 0 }}>
            ...
            />
            <Tooltip
              cursor={{ stroke: "rgba(15,23,42,0.12)", strokeWidth: 1, strokeDasharray: "3 4" }}
      ...
      <div className="glass-card-content mt-5 grid grid-cols-3 gap-4 border-t border-slate-200/60 pt-4">
        <div>
          <p className="text-xs text-slate-500">Total Revenue</p>
          <p className="mt-1 text-sm font-semibold text-slate-900">$254,800</p>
        </div>
        <div>
          <p className="text-xs text-slate-500">Orders</p>
          <p className="mt-1 text-sm font-semibold text-slate-900">1,248</p>
        </div>
        <div>
          <p className="text-xs text-slate-500">Avg. Order</p>
          <p className="mt-1 text-sm font-semibold text-slate-900">$204.3</p>
        </div>
      </div>
    </article>
  );
