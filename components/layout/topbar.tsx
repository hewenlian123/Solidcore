...
export function TopBar({ onOpenSidebar }: TopBarProps) {
  ...
  const totalAlertCount = useMemo(
    () =>
      ...
      alerts.overdueDeliveriesCount +
      alerts.specialOrdersFollowupCount,
    [alerts],
  );
    ...
    <header className="glass-bar sticky top-0 z-20">
      <div className="flex h-16 items-center justify-between gap-4 px-6 md:px-10">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-semibold tracking-tight text-white/80 md:text-base">
            overview
          </h1>
        </div>

        <div className="flex items-center gap-2">
          <span className="px-3 text-xs font-medium text-white/70">{role}</span>
                    ...
                    alerts.lowStockTop.map((item) => (
                      ...
                      >
                        ...
                        <p className="mt-0.5 text-white/50">
                          SKU {item.sku} · Available {item.available} / Reorder{" "}
                          {item.reorderLevel}
                        </p>
                      </Link>
                    ))
